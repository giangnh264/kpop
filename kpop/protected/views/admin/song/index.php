...
$this->widget('application.widgets.admin.grid.CGridView', array(
    ...
    'dataProvider' => $model->search($lyric_filter,isset($_GET['AdminSongModel']['genre_id'])?$_GET['AdminSongModel']['genre_id']:null),
        ...
        array(
        	'name' => 'Thể loại',
            'value' => '$data->songgenre->genre_name',
		...
		/* array(
			'header'=>'Tác Quyền',
			//'value'=>'@AdminCopyrightModel::model()->findByPk(AdminSongCopyrightModel::model()->findByAttributes(array("song_id"=>$data->id, "type"=>0, "active"=>1))->copryright_id)->appendix_no'
			'value'=>'CHtml::tag("div", array("id"=>"TQ_{$data->id}", "class"=>"TQ"))',
			'type'=>'raw',
		),
		array(
			'header'=>'Quyền Liên Quan',
			//'value'=>'@AdminCopyrightModel::model()->findByPk(AdminSongCopyrightModel::model()->findByAttributes(array("song_id"=>$data->id, "type"=>1, "active"=>1))->copryright_id)->appendix_no'
			'value'=>'CHtml::tag("div", array("id"=>"QLQ_{$data->id}", "class"=>"QLQ"))',
			'type'=>'raw',
		), */
		...
		array(
						'class'=>'CLinkColumn',
						'header'=>'Lyrics',
						'labelExpression'=>'(!isset($data->songextra->lyrics) || $data->songextra->lyrics=="")?CHtml::image(Yii::app()->request->baseUrl."/css/img/lyric.png", "#", array("id"=>$data->id)):CHtml::image(Yii::app()->request->baseUrl."/css/img/lyric_new.png", "#", array("id"=>$data->id))',
						'urlExpression'=>'Yii::app()->createUrl("/song/lyric", array("id"=>$data->id))',
