...
$this->menu=array(	
    ...
    array('label'=>'Thêm mới', 'url'=>"#", 'visible'=>UserAccess::checkAccess('RbtNewCreate'),'linkOptions'=>array('onclick'=>'popuprbt()')),
);
$this->pageLabel = Yii::t("admin","Danh sách ringtune mới");
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('rbt-new-model-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
{
   $.post('".$this->createUrl('rbtNew/reorder')."',$('.adminform').serialize(), function(data){
...
if(Yii::app()->user->hasFlash('RbtNew')){
    echo '<div class="flash-success">'.Yii::app()->user->getFlash('RbtNew').'</div>';
}


$this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'rbt-new-model-grid',
...
//		'status',
        ...
        array(
            ...
            'urlExpression'=>'($data->status==1)?Yii::app()->createUrl("rbtFeature/unpublish",array("cid[]"=>$data->id)):Yii::app()->createUrl("rbtFeature/publish",array("cid[]"=>$data->id))',
		...
		array(
            ...
            'header'=>CHtml::dropDownList('pageSize',$pageSize,array(10=>10,30=>30,50=>50,100=>100),array(
                                                                                  'onchange'=>"$.fn.yiiGridView.update('admin-feature-rbt-model-grid',{ data:{pageSize: $(this).val() }})",
                                                                                )),
