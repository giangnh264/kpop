...
if (is_array($this->time)) {
    if ($this->time['from'] != $this->time['to']) {
        $this->pageLabel = Yii::t('admin', "Monitoring hủy từ ngày: {from} tới {to}", array('{from}' => $this->time['from'], '{to}' => $this->time['to']));
    } else {
        $this->pageLabel = Yii::t('admin', "Monitoring hủy ngày: " . $this->time['from']);
    }
} else {
    $this->pageLabel = Yii::t('admin', "Monitoring hủy ngày: " . $this->time);
}
?>
...
<div class="content-body grid-view">
    ...
    <table width="100%" class="items">
        <tr>
            ...
            <th height="25" style="vertical-align: middle; color: #000">Failed</th>
            <th height="25" style="vertical-align: middle; color: #000">success</th>
        </tr>
        <?php
        $total = 0;
        $total_balance = 0;
