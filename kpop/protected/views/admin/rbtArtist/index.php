...
$this->pageLabel = Yii::t("admin","Danh sách RbtArtist");
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('rbt-artist-model-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
{
   $.post('".$this->createUrl('rbtArtist/reorder')."',$('.adminform').serialize(), function(data){
...
if(Yii::app()->user->hasFlash('RbtArtist')){
    echo '<div class="flash-success">'.Yii::app()->user->getFlash('RbtArtist').'</div>';
}


$this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'rbt-artist-model-grid',
...
//		'artist_song_id',
		...
		array(
            ...
            'header'=>CHtml::dropDownList('pageSize',$pageSize,array(10=>10,30=>30,50=>50,100=>100),array(
                                                                                  'onchange'=>"$.fn.yiiGridView.update('rbt-artist-model-grid',{ data:{pageSize: $(this).val() }})",
                                                                                )),
