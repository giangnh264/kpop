...
//	array('label'=> Yii::t("admin","Thêm mới"), 'url'=>array('create'), 'visible'=>UserAccess::checkAccess('SongFreeCreate')),
    array('label'=>'Thêm mới', 'url'=>"#", 'visible'=>UserAccess::checkAccess('SongFreeCreate'),'linkOptions'=>array('onclick'=>'popupsong()')),
);
$this->pageLabel = Yii::t("admin","Danh sách SongFree");
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('song-free-model-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
{
   $.post('".$this->createUrl('songFree/reorder')."',$('.adminform').serialize(), function(data){
...
if(Yii::app()->user->hasFlash('SongFree')){
    echo '<div class="flash-success">'.Yii::app()->user->getFlash('SongFree').'</div>';
}


$this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'song-free-model-grid',
	...
	'columns'=>array(
        ...
        array(
            ...
            'urlExpression'=>'($data->is_new==1)?Yii::app()->createUrl("songFree/unnew",array("cid[]"=>$data->id)):Yii::app()->createUrl("songFree/new",array("cid[]"=>$data->id))',
        ...
        array(
            ...
            'urlExpression'=>'($data->is_hot==1)?Yii::app()->createUrl("songFree/unhot",array("cid[]"=>$data->id)):Yii::app()->createUrl("songFree/hot",array("cid[]"=>$data->id))',
            'linkHtmlOptions'=>array(
                                ),
		...
		*/
		array(
            ...
            'header'=>CHtml::dropDownList('pageSize',$pageSize,array(10=>10,30=>30,50=>50,100=>100),array(
                                                                                  'onchange'=>"$.fn.yiiGridView.update('song-free-model-grid',{ data:{pageSize: $(this).val() }})",
                                                                                )),
