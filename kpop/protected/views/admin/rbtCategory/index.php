...
$this->pageLabel = Yii::t("admin","Danh sách RbtCategory");
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('rbt-category-model-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
{
   $.post('".$this->createUrl('rbtCategory/reorder')."',$('.adminform').serialize(), function(data){
...
if(Yii::app()->user->hasFlash('RbtCategory')){
    echo '<div class="flash-success">'.Yii::app()->user->getFlash('RbtCategory').'</div>';
}


$this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'rbt-category-model-grid',
...
//		'order_number',
                array(
	              ...
	              'value'=> 'CHtml::textField("order_number[$data->id]", $data->order_number,array("size"=>1))',
	              'type' => 'raw',
                ),
//		'status',
                array(
                    ...
                    'urlExpression'=>'($data->status==1)?Yii::app()->createUrl("rbtCategory/unpublish",array("cid[]"=>$data->id)):Yii::app()->createUrl("rbtCategory/publish",array("cid[]"=>$data->id))',
		...
		*/
		array(
            ...
            'header'=>CHtml::dropDownList('pageSize',$pageSize,array(10=>10,30=>30,50=>50,100=>100),array(
                                                                                  'onchange'=>"$.fn.yiiGridView.update('rbt-category-model-grid',{ data:{pageSize: $(this).val() }})",
                                                                                )),
