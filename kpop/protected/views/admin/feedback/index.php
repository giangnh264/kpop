...
$this->pageLabel = Yii::t("admin","Danh sách Feedback");
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('admin-feedback-model-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
if(Yii::app()->user->hasFlash('Feedback')){
    echo '<div class="flash-success">'.Yii::app()->user->getFlash('Feedback').'</div>';
}


$this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'admin-feedback-model-grid',
	...
	'columns'=>array(
		...
		array(
			'name'=>'type',
			'value'=>'str_replace(array(0,1,2), array("Góp ý", "Báo lỗi", "Yêu cầu chức năng"), $data->type)'
		),
		/*
		'created_datetime',
		'version',
		...
		*/
		...
		array(
            ...
            'header'=>CHtml::dropDownList('pageSize',$pageSize,array(10=>10,30=>30,50=>50,100=>100),array(
                                                                                  'onchange'=>"$.fn.yiiGridView.update('admin-feedback-model-grid',{ data:{pageSize: $(this).val() }})",
                                                                                )),
