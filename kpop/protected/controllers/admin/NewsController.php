    ...
    */
	...
	function createImage($model,$image_path)
	{
		if(file_exists($image_path)){
			...
			$s3Path = $model->getAvatarPath($model->id,"s3",false);// Thumb
			$s4Path = $model->getAvatarPath($model->id,"s4",false);// Thumb
			$s5Path = $model->getAvatarPath($model->id,"s5",false);// Thumb
			$fileSystem->mkdirs(dirname($s0Path));
			$fileSystem->mkdirs(dirname($s1Path));
			$fileSystem->mkdirs(dirname($s2Path));
			$fileSystem->mkdirs(dirname($s3Path));
			$fileSystem->mkdirs(dirname($s4Path));
			$fileSystem->mkdirs(dirname($s5Path));

			list($width, $height) = getimagesize($image_path);
			$imgCrop = new ImageCrop($image_path, 0, 0, $width, $height);

			$fileSystem->copy($image_path, $s0Path);
			$imgCrop->resizeFix($s1Path, 600, 360);
			$imgCrop->resizeFix($s2Path, 320, 200);
			$imgCrop->resizeFix($s3Path, 160, 100);
			$imgCrop->resizeFix($s4Path, 100, 100);
			$imgCrop->resizeFix($s5Path, 100, 100);
