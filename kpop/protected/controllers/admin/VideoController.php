...
class VideoController extends Controller
{
	...
	{
		$metaModel = AdminVideoMetadataModel::model()->findByPk($id);
	...
	public function actionCreate()
	{
		...
		$videoMeta = new AdminVideoMetadataModel;

        $valcopy0 = Yii::app()->request->getParam('valcopy0');
        $valcopy1 = Yii::app()->request->getParam('valcopy1');
		$tagContent = new TagContentModel();
		$tagContent->setAttribute("type","video");
		...
		if(isset($_POST['AdminVideoModel']))
		{
			...
			//check exits file
			$videoFile = _APP_PATH_.DS."data".DS."tmp".DS.$_POST['tmp_source_path'];
			if(file_exists($videoFile)){
				...
				if($model->save()){
					...
					$videoExtra->video_id = $model->id;
					$videoExtra->description = $_POST['AdminVideoModel']['description'];
					$videoExtra->save();
	...
	public function actionUpdate($id)
	{
		...
		/*
		$_GET['msg'] = Yii::t('admin','Chức năng upload video tạm thời dừng để nâng cấp hệ thống');
     	$this->forward("admin/error",true);
     	*/
        $valcopy0 = Yii::app()->request->getParam('valcopy0');
        $valcopy1 = Yii::app()->request->getParam('valcopy1');
        $cr = new CDbCriteria();
        $cr->condition = "t.video_id = :id ";
		...
		$videoMeta = AdminVideoMetadataModel::model()->findByPk($id);
		if(empty($videoMeta)){
			$videoMeta = new AdminVideoMetadataModel();
			$videoMeta->video_id = $id;
		}

		$videoExtra = AdminVideoExtraModel::model()->findByPk($id);
		if(empty($videoExtra)){
			$videoExtra = new AdminVideoExtraModel();
			$videoExtra->video_id = $id;
		}
		...
		if(isset($_POST['AdminVideoModel']))
		{
			...
			$videoMeta->attributes = $_POST['videoMeta'];
			...
			if(isset($_FILES['avatar_upload'])){
				if($_FILES['avatar_upload']['type']== 'image/jpeg' ){
					...
					$tmpPath = _APP_PATH_.DS."public/admin/data/tmp".DS.time().".jpg";
			...
			if($model->save()){
                // Copyright
                $cr = new CDbCriteria();
                $cr->condition = "video_id = {$model->id}";
    ...
    */
	...
	public function actionConfirmDel()
	{
		...
		if(Yii::app()->getRequest()->ispostRequest && $isPopup == 1){
			...
			if(!empty($videoMass)){
				...
				AdminVideoModel::model()->setdelete($this->userId, $reason, $videoMass, $status);
			}else{
    ...
    public function actionApprovedAndApplyVideo($id)
    {
    	...
    	try{
	    	...
	    	$videoMeta = AdminVideoMetadataModel::model()->findByPk($videoId);
	    	if(empty($videoMeta)){
	    		$videoMeta = new AdminVideoMetadataModel();
	    		$videoMeta->video_id = $videoId;
	    	}

	    	$videoExtra = AdminVideoExtraModel::model()->findByPk($videoId);
	    	if(empty($videoExtra)){
	    		$videoExtra = new AdminVideoExtraModel();
	    		$videoExtra->video_id = $videoId;
	    	}
	    	...
	    	if(isset($dataPost['AdminVideoModel']))
	    	{
	    		...
	    		//check exits file
	    		$videoFile = _APP_PATH_.DS."data".DS."tmp".DS.$dataPost['tmp_source_path'];
	    		if(file_exists($videoFile)){
	    			$this->moveFile($model,$videoFile);
	    			$data['approved_by'] = 0;
	    		...
	    		$removeAtt[] = 'source_path';
	    		$model->unsetAttributes($removeAtt);
	    		$videoMeta->attributes = $dataPost['videoMeta'];
	    		...
	    		if($model->save()){
	    			...
	    			//Update video Extra
	    			$videoExtra->description = $dataPost['AdminVideoModel']['description'];
	    			$videoExtra->save();
