...
class SongController extends Controller {
    ...
    public function actionIndex() {
        ...
        $is_composer="";
        if(!$_GET['AdminSongModel']['composer_id']){
    ...
    public function actionView($id) {
        ...
        $metaModel = AdminSongMetadataModel::model()->findByPk($id);
    ...
    public function actionCreate() {
        ...
        $tagContent->setAttribute("type","song");
        if (isset($_POST['AdminSongModel'])) {
            ...
            //check exits file
            $fileMp3 = _APP_PATH_ . DS . "data" . DS . "tmp" . DS . $_POST['tmp_source_path'];
            ...
            //if (file_exists($fileMp3) && !$isExits) {
                if ($model->save()) {
                    ...
                    $songExtra->song_id = $model->id;
                    $songExtra->lyrics = $_POST['AdminSongExtraModel']['lyrics'] ? $_POST['AdminSongExtraModel']['lyrics'] : "";
                    $songExtra->save();
                    ...
                    $tagContent->type_id = $model->id;
                    $tagContent->tag_name = $_POST['tag_name'];
                    $tagContent->type = 'song';
                    $tagContent->type_id = $_POST['AdminSongModel']['name'];
                    $tagContent->save();
    ...
    public function actionUpdate($id) {
        ...
        $songMeta = AdminSongMetadataModel::model()->findByPk($id);
        if (empty($songMeta)) {
            $songMeta = new AdminSongMetadataModel();
            $songMeta->song_id = $id;
        }

        $songExtra = AdminSongExtraModel::model()->findByPk($id);
        if (empty($songExtra)) {
            $songExtra = new AdminSongExtraModel;
            $songExtra->song_id = $id;
        }
        if (isset($_POST['AdminSongModel'])) {
    ...
    public function actionConfirmDel() {
        ...
        if (Yii::app()->getRequest()->ispostRequest && $isPopup == 1) {
            ...
            if (!empty($songMass)) {
                ...
                AdminSongModel::model()->setdelete($this->userId, $reason, $songMass, $status);
            } else {
    ...
    public function actionGetcopy(){
        ...
        foreach ($copyright as $item){
            $tmp = array();
            $tmp['id'] = $item['id'];
            $tmp['appendix_no'] = $item['contract_no'];
	...
	public function actionApprovedAndApplySong()
	{
		...
		try{
			...
			$songExtra = AdminSongExtraModel::model()->findByPk($songId);
			if (empty($songExtra)) {
				$songExtra = new AdminSongExtraModel;
				$songExtra->song_id = $songId;
			}
			if (isset($dataPost['AdminSongModel'])) {
				...
				if ($model->save()) {
					...
					//Update song extra
					$songExtra->lyrics = $dataPost['AdminSongExtraModel']['lyrics'] ? $dataPost['AdminSongExtraModel']['lyrics'] : "";
					$songExtra->save();
