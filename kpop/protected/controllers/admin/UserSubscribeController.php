...
class UserSubscribeController extends Controller
{
	...
	public function actionIndex()
	{
		...
		$msisdn = $_GET['AdminUserSubscribeModel']['user_phone'];
		if(empty($msisdn)){
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['AdminUserSubscribeModel']))
		{
			...
			if(!Formatter::isPhoneNumber(Formatter::removePrefixPhone($phone))){
				$model->addError('phone', "Thuê bao {$phone} không phải của Mobifone");
			}else{
				try {
	...
	public function actionUpdate($id)
	{
		 ...
		 if(isset($_POST['expired_time']))
		 {
			$model->expired_time=$_POST['expired_time'];
			if($model->save())
	...
	public function actionTryRegister($phone)
	{
		...
		if(!Formatter::isPhoneNumber(Formatter::removePrefixPhone($phone))){
			echo  "Thuê bao {$phone} không phải của Mobifone";
			exit;
		}
	...
	public function actionVip()
	{
		if(Yii::app()->getrequest()->ispostrequest){
			...
			if(Formatter::isVinaphoneNumber($phone)){
				...
				Yii::app()->user->setFlash('uservip',"Số điện thoại '{$phone}' không phải TB Mobifone");
			}
		}
		$pageSize = Yii::app()->request->getParam('pageSize',Yii::app()->params['pageSize']);
		Yii::app()->user->setState('pageSize',$pageSize);
