...
/**
 * This is the model class for table "video_artist".
 *
 * The followings are the available columns in table 'video_artist':
 ...
 */
class BaseVideoArtistModel extends MainActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('video_id, artist_id', 'required'),
			array('video_status, ordering', 'numerical', 'integerOnly'=>true),
			array('video_id, artist_id', 'length', 'max'=>10),
			array('artist_name', 'length', 'max'=>255),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('video_id, artist_id, artist_name, video_status, ordering', 'safe', 'on'=>'search'),
		);
	}
