<?php

/**
 * This is the model class for table "song_profile".
 *
 * The followings are the available columns in table 'song_profile':
 * @property integer $id
 * @property string $profile_id
 * @property string $name
 * @property string $quality_name
 * @property string $format
 * @property string $codec
 * @property string $bitrate
 * @property string $param
 * @property integer $channel
 * @property string $samplerate
 * @property integer $sorder
 * @property integer $http_support
 * @property integer $rtsp_support
 * @property integer $rtmp_support
 * @property integer $status
 */
class BaseSongProfileModel extends MainActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @return SongProfile the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}

	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 'song_profile';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('profile_id, name', 'required'),
			array('channel, sorder, http_support, rtsp_support, rtmp_support, status', 'numerical', 'integerOnly'=>true),
			array('profile_id, codec, bitrate, samplerate', 'length', 'max'=>10),
			array('name', 'length', 'max'=>45),
			array('quality_name', 'length', 'max'=>20),
			array('format', 'length', 'max'=>4),
			array('param', 'length', 'max'=>255),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, profile_id, name, quality_name, format, codec, bitrate, param, channel, samplerate, sorder, http_support, rtsp_support, rtmp_support, status', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
            return Common::loadMessages("db");
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 * @return CActiveDataProvider the data provider that can return the models based on the search/filter conditions.
	 */
	public function search()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('id',$this->id);
		$criteria->compare('profile_id',$this->profile_id,true);
		$criteria->compare('name',$this->name,true);
		$criteria->compare('quality_name',$this->quality_name,true);
		$criteria->compare('format',$this->format,true);
		$criteria->compare('codec',$this->codec,true);
		$criteria->compare('bitrate',$this->bitrate,true);
		$criteria->compare('param',$this->param,true);
		$criteria->compare('channel',$this->channel);
		$criteria->compare('samplerate',$this->samplerate,true);
		$criteria->compare('sorder',$this->sorder);
		$criteria->compare('http_support',$this->http_support);
		$criteria->compare('rtsp_support',$this->rtsp_support);
		$criteria->compare('rtmp_support',$this->rtmp_support);
		$criteria->compare('status',$this->status);

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=> Yii::app()->user->getState('pageSize',Yii::app()->params['pageSize']),
			),
		));
	}
}