...
class BaseArtistModel extends MainActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('genre_id, artist_type_id, created_by, updated_by, sorder, status, cmc_id', 'numerical', 'integerOnly'=>true),
			array('old_id, song_count, video_count, album_count, fan_count', 'length', 'max'=>10),
			array('name, url_key', 'length', 'max'=>160),
			array('map_name', 'length', 'max'=>300),
			array('artist_key', 'length', 'max'=>45),
			array('song_ids', 'length', 'max'=>100),
			array('description, created_time, updated_time', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, old_id, name, url_key, description, song_count, video_count, album_count, fan_count, genre_id, artist_type_id, map_name, created_by, updated_by, created_time, updated_time, sorder, status, artist_key, song_ids, cmc_id', 'safe', 'on'=>'search'),
		);
	}
