...
/**
 * This is the model class for table "log_api_provider".
 *
 * The followings are the available columns in table 'log_api_provider':
 * @property integer $id
 ...
 */
class BaseLogApiProviderModel extends MainActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('loged_time, client_ip, service_name, service_url, start_time, response', 'required'),
			array('execute_time', 'numerical', 'integerOnly'=>true),
			array('client_ip, service_name, service_url, request_params', 'length', 'max'=>255),
			array('protocol', 'length', 'max'=>30),
			array('end_time', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, loged_time, client_ip, service_name, service_url, request_params, protocol, start_time, end_time, execute_time, response', 'safe', 'on'=>'search'),
		);
	}
