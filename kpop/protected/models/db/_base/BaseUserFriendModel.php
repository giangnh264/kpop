 ...
 */
class BaseUserFriendModel extends MainActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('friend_id', 'required'),
			array('friend_id', 'numerical', 'integerOnly'=>true),
			array('user_id', 'length', 'max'=>11),
			array('old_id', 'length', 'max'=>10),
			array('created_time', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, user_id, old_id, friend_id, created_time', 'safe', 'on'=>'search'),
		);
	}
