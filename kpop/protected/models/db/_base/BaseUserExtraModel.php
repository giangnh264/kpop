...
class BaseUserExtraModel extends MainActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('user_id, deleted_by, flag_username, point', 'numerical', 'integerOnly'=>true),
			array('introduction', 'length', 'max'=>500),
			array('cmnd', 'length', 'max'=>20),
			array('deleted_reason', 'length', 'max'=>255),
			array('birthday, updated_time', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('user_id, introduction, cmnd, deleted_by, deleted_reason, birthday, flag_username, point, updated_time', 'safe', 'on'=>'search'),
		);
	}
