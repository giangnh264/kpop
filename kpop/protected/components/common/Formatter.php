...
class Formatter{
    public static function formatPhone($phone){
		...
		}elseif (strpos($phone,'089') === 0){
            $phone=substr($phone,1);
            $phone=Yii::app()->params['phone.country.code'].$phone;
        } elseif (strpos($phone,'89') === 0){
            $phone=Yii::app()->params['phone.country.code'].$phone;
        }
		return trim($phone);
    }
	...
	public static function isPhoneNumber($phone) {
		...
		$pattern = "/^(089|090|093|0122|0126|0128|0121|0120)([0-9]{7})$/";
		return preg_match($pattern,$phone);
	}

	public static function isMobiPhoneNumber($phone) {
		$phone = trim($phone);
		if(strpos($phone,"84")===0){
			$phone = "0".substr($phone,"2");
		}else if(strpos($phone,"0")!==0){
...
//        $dayOfWeekVn = array('Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday');//
		$dayOfWeekVn = array('Thứ hai','Thứ ba','Thứ tư','Thứ năm','Thứ sáu','Thứ bảy','Chủ nhật');
